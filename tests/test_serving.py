...
def client(tmp_path_factory):
    os.environ.setdefault("MULE_API_KEYS", "demo-investigator-key")
    os.environ.setdefault("MULE_ADMIN_API_KEYS", "demo-admin-key")
    os.environ.setdefault("MULE_ALLOW_DEMO_AUTH", "true")
    os.environ["MULE_AUDIT_DB_PATH"] = str(tmp_path_factory.mktemp("audit") / "mule_audit.sqlite3")
    from fastapi.testclient import TestClient
    from serving.app import app
    return TestClient(app, headers={"X-API-Key": "demo-investigator-key"})
